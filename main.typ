...
#show: book.with(
  ...
  include-preface: true, // 是否引入前言言（默认引入：true，如果不需要前言页可以填写false）
  outline-image-index1: image("./fig/orange1.jpg"), // 目录头图（不需要可以注释掉）
  list-of-figure-title: "图像目录", // 图像索引（不需要可以注释掉）
  outline-image-index2: image("./fig/orange1.jpg"), // 图像索引头图（不需要可以注释掉）
  list-of-table-title: "表格目录", // 表格索引（不需要可以注释掉）
  outline-image-index3: image("./fig/orange1.jpg"), // 表格索引头图（不需要可以注释掉）
  ...
  par-leading: 0.5em, // 行间距（默认0.5倍）
  par-spacing: 0.5em, // 段间距（默认0.5倍）
  copyright: [
    Copyright © 2024 打字机
