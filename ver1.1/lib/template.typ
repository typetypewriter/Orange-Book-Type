...
#let book(title: "", subtitle: "", author: (), paper-size: "a4", logo: none, cover: none, preface-parent-name: none, include-preface: none, outline-image-index: none, body, main-color: rgb("#F36619"), copyright: [], lang: "zh", list-of-figure-title: none, fig-outline-image-index: none, list-of-table-title: none, tab-outline-image-index: none, supplement-chapter: "Chapter", supplement-part: "Part", par-indent: 2em, par-justify: true, par-leading: 1em, par-spacing: 1em, font-size: 10pt, part-style: 0, uplower-case-references: none) = {
  ...
  /*是否引入前言*/ 
  if (include-preface != none){
    let no-hide = (preface-parent-name == none)
    show: prefaces.with(preface-parent-name, hide-parent: no-hide)
    show link: set text(fill: main-color) // 链接样式
    if type(include-preface) == str {
      include include-preface
    }
    if type(include-preface) == array {
      for file in include-preface {
        include file
      }
    }
  }
  ...
  /*制作全书目录页*/
  update-heading-image(image: outline-image-index)
